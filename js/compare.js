$(document).ready(function () {
    ...
    //hien thi toan bo hinh anh + ten + gia truoc khi so sanh
    function displayImages(items) {
        let s = ``;

        $.each(items, function (k, v) {

            s += `<div class="col-sm-6 col-md-4 divImage" ${v.type}  ${v.brand}>  
                <a href = "#${v.id}" target = "_blank"><img src="images/${v.link}" class="img-responsive ep_fade ep_fade_0 card" style="width:100%" alt=""></a> 
                <p>${v.name}</p>
                <p>${v.cost}</p>
        ...
        if (mark.length < 2 || mark.length > 4) {
            ...
            for (var i = 0; i < mark.length; ++i) {
                $("#product").append(
                    '<div class="col-xs-' + n + '">' +
                    '<div class="row border rowImage">' +
                    '<img src="images\\' + arr[i].link + '" alt="" height = "100%">' +
                    '</div>' +
                    '<div class="row border">' +
                    '<p>' + arr[i].name + '</p><br>' +
                    '</div>' +
